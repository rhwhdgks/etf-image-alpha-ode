...
|---|---|---|---|
| 1 | **Walk-forward folds have no horizon embargo.** The original `generate_walkforward_folds` placed train/val/test adjacently; the last ~20 fitted dates per fold could carry 20-day forward-return labels that overlap the adjacent test period. | Medium | **Sensitivity-tested and code path fixed for future retraining.** `outputs/ode_handoff/06_mu_submission_validation/` drops the first 20 OOS dates of every 60-day fold and reruns locked-candidate metrics plus block bootstrap. `PipelineConfig.wf_embargo_days` and `--wf-embargo-days` were added to the walk-forward/image-factor runners. A 1-fold/1-epoch purged retraining smoke test passed and produced `cnn_training_history.csv`. Full 48-fold/30-epoch purged retraining is still the stricter journal-level final run. |
