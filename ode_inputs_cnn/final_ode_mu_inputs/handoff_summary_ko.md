...
| `final_mu_inputs_long.csv` | 4개 후보 전체, long format |
| `final_mu_inputs_wide.csv` | 4개 후보 전체, wide format |
...
5. 비교 대상으로 `mu_sharpe_baseline`, `mu_rank_baseline`, `mu_image_factor_balanced`도 돌린다.
...
ODE 팀에는 우선 `selected_mu_input.csv`의 `mu_signal`을 `mu(t)` 후보로 넘기면 된다. `Sigma(t)`는 같은 폴더의 `sigma_wide.csv` 또는 `sigma_long.csv`를 쓰면 된다. 이 input은 원자료 baseline보다 훨씬 강하고, 기존 `ensemble_4family`보다 rank corr가 높다. 다만 Sharpe는 낮아졌으므로 최종 판단은 ODE optimizer 내부의 risk control과 함께 비교해야 한다.
