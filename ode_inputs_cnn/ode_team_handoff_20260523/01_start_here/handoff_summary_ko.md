...
이 폴더는 이미지 팩터 실험 결과를 바탕으로 ODE 포트폴리오 최적화 팀에 넘길 `mu(t)`, `Sigma(t)`, realized return 입력값을 한 곳에 모은 전달 패키지다.

Optimization과 ODE solver 실행은 아직 포함하지 않았다. 여기서는 `date-asset`별 expected-return ranking signal과 ODE 검증에 필요한 covariance/return 자료를 제공한다.
...
```text
ode_inputs_cnn/ode_team_handoff_20260523/
```

ODE 팀이 먼저 보면 되는 파일:

```text
ode_inputs_cnn/ode_team_handoff_20260523/02_mu_inputs/selected_mu_input.csv
```
...
| 파일 | 용도 |
|---|---|
| `02_mu_inputs/selected_mu_input.csv` | 추천 input 하나만 들어있는 파일. ODE 팀은 여기서 시작하면 된다. |
| `02_mu_inputs/final_mu_inputs_long.csv` | 5개 후보 전체, long format |
| `02_mu_inputs/final_mu_inputs_wide.csv` | 5개 후보 전체, wide format |
| `02_mu_inputs/final_mu_inputs_calibrated.csv` | expanding-window 회귀로 return-scale로 보정한 `mu(t)` 후보 |
| `03_sigma_returns/sigma_wide.csv` | `mu(t)` 날짜와 정렬된 일별 `Sigma(t)` wide format |
| `03_sigma_returns/sigma_long.csv` | `date-asset_i-asset_j` 단위의 full covariance matrix long format |
| `03_sigma_returns/returns_for_ode.csv` | `mu(t)`, `Sigma(t)`와 정렬된 실제 일별 수익률 |
| `02_mu_inputs/input_performance_summary.csv` | 후보별 성능 요약 |
| `02_mu_inputs/main_vs_strict_mu_comparison.csv` | 기존 main image factor와 strict-window MA 후보 직접 비교 |
| `02_mu_inputs/mu_calibration_summary.csv` | calibrated `mu`의 coverage, RMSE, rank corr 요약 |
| `01_start_here/README.md` | 영어 설명, 컬럼 설명, 사용법 |
| `01_start_here/manifest.json` | machine-readable metadata |
| `03_sigma_returns/sigma_manifest.json` | Sigma 관련 metadata |

## Sigma(t)도 있는가?

있다. 최종 전달 패키지 안에 `Sigma(t)`도 같이 정리해두었다.

| 파일 | 크기/형태 | 설명 |
|---|---:|---|
| `03_sigma_returns/sigma_wide.csv` | 2,880 rows x 29 columns | 날짜별 7개 variance와 21개 covariance |
| `03_sigma_returns/sigma_long.csv` | 141,120 rows x 5 columns | 날짜별 7 x 7 covariance matrix 전체 |
| `03_sigma_returns/returns_for_ode.csv` | 2,880 rows x 8 columns | 실제 일별 수익률, backtest/realized PnL용 |
...
주의할 점은 Sigma는 image factor 모델에서 새로 예측한 값이 아니라, 과거 수익률 기반 rolling covariance다. 즉 ODE 팀은 `02_mu_inputs/selected_mu_input.csv`에서 `mu(t)`를 받고, `03_sigma_returns/sigma_wide.csv` 또는 `03_sigma_returns/sigma_long.csv`에서 `Sigma(t)`를 받으면 된다.

## 패키지 내부 근거 파일 위치

| 목적 | 파일 |
|---|---|
| 전체 패키지 안내 | `README_ODE_TEAM.md` |
| ODE용 시작 README | `01_start_here/README.md` |
| image factor Phase 1-2 요약 | `05_source_notes/image_factor_phase12_final_report.md` |
| Strict-window MA robustness 결과 | `05_source_notes/strict_window_ma_result_note.md` |
| 30개 ETF pretraining negative result | `05_source_notes/extra_pretraining_result_note.md` |
| Multiple-testing 보정 결과 | `04_validation_reports/multiple_testing_report.md` |
| Multiple-testing 전체 CSV | `04_validation_reports/image_factor_significance_multiple_testing.csv` |
| Subperiod stability 결과 | `04_validation_reports/mu_subperiod_stability_report.md` |
| Subperiod stability CSV | `04_validation_reports/mu_subperiod_stability.csv` |
| Return-scale calibration 결과 | `04_validation_reports/mu_calibration_report.md` |
| Strict-window MA ODE 후보 비교 | `02_mu_inputs/main_vs_strict_mu_comparison.csv` |
...
1. `02_mu_inputs/selected_mu_input.csv`를 읽는다.
2. `date × asset` wide matrix로 pivot한다.
3. `mu_signal`을 `mu(t)` 후보로 사용한다.
4. `03_sigma_returns/sigma_wide.csv` 또는 `03_sigma_returns/sigma_long.csv`에서 `Sigma(t)`를 읽는다.
...
mu = pd.read_csv("ode_inputs_cnn/ode_team_handoff_20260523/02_mu_inputs/selected_mu_input.csv", parse_dates=["date"])
mu_wide = mu.pivot(index="date", columns="asset", values="mu_signal")

sigma = pd.read_csv("ode_inputs_cnn/ode_team_handoff_20260523/03_sigma_returns/sigma_wide.csv", parse_dates=["date"])
returns = pd.read_csv("ode_inputs_cnn/ode_team_handoff_20260523/03_sigma_returns/returns_for_ode.csv", parse_dates=["date"])
```

## 결론

ODE 팀에는 우선 `02_mu_inputs/selected_mu_input.csv`의 `mu_signal`을 `mu(t)` 후보로 넘기면 된다. `Sigma(t)`는 `03_sigma_returns/sigma_wide.csv` 또는 `03_sigma_returns/sigma_long.csv`를 쓰면 된다. 메인 input은 원자료 baseline보다 훨씬 강하고, 기존 `ensemble_4family`보다 rank corr가 높다. 추가로 `mu_image_factor_strict_rank`는 MA 계산을 더 보수적으로 제한한 robustness 후보이므로, ODE optimizer 안에서 main input과 함께 비교하는 것이 좋다.
