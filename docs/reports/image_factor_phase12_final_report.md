...
## 2026-05-19 Handoff Status

이 문서는 image factor 형성과 ablation 단계의 원천 요약이다. 공개 레포에서는 ODE팀 전달용 `mu(t)`, `Sigma(t)`, validation 자료를 `outputs/ode_handoff/`에 curated package 형태로 남겼다.

ODE팀이 실제로 읽어야 하는 시작 파일은 `README_ODE_TEAM.md`와 `01_start_here/handoff_summary_ko.md`다. 이 문서는 연구 근거를 확인할 때 참고하면 된다.
...
따라서 1번과 2번은 마감 가능하다. 이 결과를 바탕으로 ODE 팀에 넘길 `mu(t)` candidate input은 현재 `02_mu_inputs/`에 정리되어 있다.

## Public Files To Use

| 목적 | 파일 |
|---|---|
| ODE handoff package | `outputs/ode_handoff/` |
| 추천 `mu(t)` | `outputs/ode_handoff/02_mu_inputs/selected_mu_input.csv` |
| 후보 성능 요약 | `outputs/ode_handoff/02_mu_inputs/input_performance_summary.csv` |
| Multiple-testing 보정 | `outputs/ode_handoff/04_validation_reports/multiple_testing_report.md` |
| Subperiod stability | `outputs/ode_handoff/04_validation_reports/mu_subperiod_stability_report.md` |
| Process integrity audit | `outputs/ode_handoff/04_validation_reports/process_integrity_report.md` |
| 논문형 요약 | `docs/reports/paper_draft_ko.md` |
